...
public class MySelectQueryTest2 {
	Connection conn;

	@Before
	public void setUp() throws Exception {
		conn = DriverManager.getConnection("jdbc:solr:http://localhost:8983/solr/collection1");
	}
	...
	public void testStatement() throws SQLException {
		...
		verifyStatement("SELECT player_name FROM player2 ORDER BY player_id",
				expected);
	}
	...
	public void testStatementLimit() throws SQLException {
		...
		verifyStatement("SELECT player_name FROM player2 ORDER BY player_id LIMIT 3 OFFSET 1",
				expected);
	}
	...
	public void testStatementOrderBy() throws SQLException {
		...
		verifyStatement("SELECT player_name FROM player2 ORDER BY player_id DESC",
				expected);
	}
	...
	public void testStatementCondition() throws SQLException {
		...
		verifyStatement("SELECT player_name FROM player2 WHERE player_id > 1 AND player_id < 3",
				expected1);
	...
	public void testStatementOr() throws SQLException {
		...
		verifyPreparedStatement(
				"SELECT player_name FROM player2 WHERE (player_id = 1 OR player_id = 5) AND team=?",
				params,
				expected1);
	}
	...
	public void testStatementCount() throws SQLException {
		Object[][] expected = {{"5"}};
		verifyStatement("SELECT count(*) FROM player2", expected);
	}

	@Test
	public void testStatementGroupBy() throws SQLException {
		Object[][] expected = {{"カープ", "4"}, {"阪神", "1"}};
		verifyStatement("SELECT team, count(*) FROM player2 GROUP BY team", expected);
	}
	...
	public void testStatementIn() throws SQLException {
		...
		verifyPreparedStatement(
				"SELECT player_name FROM player2 WHERE position in (?,?) order by player_id",
				params,
				expected);
	}
	...
	public void testLike() throws SQLException {
		...
		Object[] params = {"衣笠%"};
		verifyPreparedStatement(
				"SELECT player_name FROM player2 WHERE player_name like ?",
				params,
				expected);
	}
	...
	public void testLikeForText() throws SQLException {
		...
		Object[] params = {"%三拍子%"};
		verifyPreparedStatement(
				"SELECT player_name FROM player2 WHERE comment like ?",
				params,
				expected);
	}
	...
	public void testBetween() throws SQLException {
		...
		Object[] params = {2,3};
		verifyPreparedStatement(
				"SELECT player_name FROM player2 WHERE player_id BETWEEN ? AND ?",
				params,
				expected);
	}
	...
	public void testBoolean() throws SQLException {
		...
		Object[] params = {true};
		verifyPreparedStatement("SELECT player_name FROM player2 WHERE was_homerun_king=?",
				params, expected);
	}
	...
	public void testQueryContainsMetachar() throws SQLException {
		...
		verifyPreparedStatement("SELECT player_name FROM player2 WHERE player_name=?",
				params, expected);
	}
	...
	public void testStatementColumnNotFound() throws SQLException {
		...
		try {
			stmt = conn.prepareStatement("select prayer_name FROM player2");
			fail("No Exception");
		} catch (SQLException e) {
	...
	public void testGetColumnLabel() throws SQLException {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery("Select player_name FROM player2 where player_id=3");
			assertTrue(rs.next());
	...
	public static void init() throws SQLException, ClassNotFoundException {
		...
		try {
			PreparedStatement dropStmt = setUpConn.prepareStatement("DROP TABLE player2");
			try {
			...
			PreparedStatement stmt = setUpConn.prepareStatement(
					"CREATE TABLE player2 (player_id number, team varchar(10), "
			...
			PreparedStatement insStmt = setUpConn.prepareStatement("INSERT INTO player2 Values (?,?,?,?,?,?,?)");
			try {
				insStmt.setInt(1, 1);
